...
// The respective loss and accuracies is stored to the file `~/output/compare/ftir-cnn.json`.
//
...
fn main() {
    ...
    let mut file = File::create("./output/compare/ftir-cnn-redo.json").unwrap();
    writeln!(file, "[").unwrap();
    writeln!(file, "  {{").unwrap();

    vec!["FB2x3"].iter().for_each(|method| {
        println!("Method: {}", method);
        vec![false].iter().for_each(|skip| {
            println!("  Skip: {}", skip);
            vec!["CLASSIFICATION"].iter().for_each(|problem| {
                ...
                for run in 1..RUNS + 1 {
                    ...
                    if problem == &"REGRESSION" {
                        ...
                        network
                            .set_objective(objective::Objective::CrossEntropy, Some((-5.0, 5.0)));
                    }
