...
// The respective loss and accuracies is stored to the file `~/output/compare/mnist.json`.
//
...
fn main() {
    ...
    let class_train = load_labels("./examples/datasets/mnist/train-labels-idx1-ubyte", 10).unwrap();
    ...
    let mut file = File::create("./output/compare/mnist.json").unwrap();
    writeln!(file, "[").unwrap();
    writeln!(file, "  {{").unwrap();
